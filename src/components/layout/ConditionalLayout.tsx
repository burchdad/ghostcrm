...
import { CollapseToggle, useCollapse } from "@/components/layout/collapse";

// Pages that should NOT show the authenticated UI (sidebar, topbar, etc.)
const PUBLIC_PAGES = [
  "/login",
  "/register", 
  "/reset-password",
...
export default function ConditionalLayout({ children }: { children: React.ReactNode }) {
  ...
  // For public pages, just render children without authenticated UI
  if (isPublicPage) {
    return <>{children}</>;
  }

  // For authenticated pages, render the full layout with sidebar, topbar, etc.
  return (
