...
export function ProgressBar({ value, max = 100, color = "bg-blue-500", showLabel = false, animated = true }: ProgressBarProps) {
  ...
  return (
    <div className="w-full bg-gray-200 rounded h-3 relative" aria-valuenow={value} aria-valuemax={max} aria-valuemin={0} role="progressbar">
      <div className={`${color} h-3 rounded transition-all ${animated ? 'animate-progress' : ''}`} style={{ width: `${percent}%` }} />
      {showLabel && (
