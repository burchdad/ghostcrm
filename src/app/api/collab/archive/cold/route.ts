...
export async function POST(request: Request) {
  ...
  if (rowCount + records.length > MAX_ROWS) {
    // Create new table
    const idx = tableName === COLD_TABLE_BASE ? 1 : parseInt(tableName.split('_').pop() || '1') + 1;
    tableName = await createNewTable(COLD_TABLE_BASE, idx);
    rowCount = 0;
  }
  // Insert into cold storage table
