...
// Compliance badges - load from API
const getComplianceBadges = async (orgId: string): Promise<string[]> => {
  try {
    const response = await fetch(`/api/organizations/${orgId}/compliance`);
    if (response.ok) {
      const data = await response.json();
      return data.badges || [];
    }
  } catch (error) {
    console.error('Failed to fetch compliance badges:', error);
  }
  return [];
};
...
// SSO configuration - load from environment
